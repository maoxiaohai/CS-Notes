...
常量池的回收：
只要常量池中的常量不被任何变量或者对象所引用，就被认定为废弃常量，当进行垃圾回收时，废弃常量就会被回收。比如，字符串常量"bingo"，如果他没有被任何变量或者对象所引用，当进行垃圾回收时就会被清除。
...
当一个对象可被回收时，如果需要执行该对象的 finalize() 方法(finalize()只能被执行一次)，那么就有可能在该方法中让对象重新被引用，从而实现自救。自救只能进行一次，如果回收的对象之前调用了 finalize() 方法自救，后面回收时不会再调用该方法。
### 5.finalize()是否需要被执行
当一个类没有重写finalize或者finalize被JVM已经执行过了，就认为是不需要执行的。当不满足这两个条件时，被认为是有必要被执行。当被认为有必要执行finalize时，那么对象会被放入一个 F-Queue 队列中，虚拟机会以较低的优先级执行这些 finalize()方法，但不会确保所有的 finalize() 方法都会执行结束。
...
- 新生代使用：复制算法(新生代进行垃圾回收更频繁)
- 老年代使用：标记 - 清除 或者 标记 - 整理 算法(老年代存活的对象占大部分，采用复制算法就太慢了)
...
## 可能发生OOM的区域
除了程序计数器不会发生OOM之外，其他区域都是可能发生OOM的。
### 1.堆内存
   堆内存不足是最常见的发送OOM的原因之一。如果在堆中没有内存完成对象的分配，并且堆无法完成拓展时，就会抛出OOM异常。当前主流的JVM可以通过-Xmx和-Xms来控制堆内存的大小，发生堆上OOM的可能是内存泄露，也可能是堆大小分配不合理。
### 2.栈
   有虚拟机栈和本地方法栈，这两个区域的区别不过是虚拟机栈为虚拟机执行Java方法服务，而本地方法栈则为虚拟机使用到的Native方法服务，在内存分配异常上是相同的。在JVM规范中，对Java虚拟机栈规定了两种异常：1：如果线程请求的栈大于所分配的栈大小，则抛出StackOverFlowError错误，比如进行了一个不会停止的递归调用。2：如果虚拟机栈是可以动态拓展的，拓展时无法申请到足够的内存，则会抛出OutOfMemoryError错误。
### 3.直接内存
   直接内存虽然不是虚拟机运行时数据区的一部分，但既然是内存，就会受到物理内存的限制，在JDK1.4中引入的NIO使用Native函数库在堆外内存上直接分配内存，但直接内存不足时，也会导致OOM。
### 4.方法区
   随着Metaspace元数据区的引入，方法区的OOM错误信息也变成java.lang.OutOfMemoryError:Metaspace。对于旧版本的Oracle JDK,由于永久代的大小限制，而JVM对永久代的垃圾回收并不积极，如果往永久代不断写入数据，例如String.Intern()的调用，在永久代占用太多空间导致内存不足，也会出现OOM的问题。
